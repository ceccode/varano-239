...
- Stato: **Accettata**
- Data: 10 agosto 2026
- Contesto: i budget di performance esistevano solo come prosa («JavaScript iniziale: massimo 150 KB gzip», QUALITY.md) e nessun cancello li verificava; axe girava una volta sola, sulla prima schermata, quindi menù, dialoghi, briefing e finali sono stati pubblicati per settimane senza un solo audit; e non esisteva alcuna misura del costo di disegno per frame, quindi una regressione di rendering non aveva modo di farsi notare in CI.
- Decisione, tre cancelli — tutti deterministici, mai basati sul tempo (i millisecondi in CI sono rumore):
  1. **Il peso del bundle entra in `npm run check`**: `scripts/check-bundle-size.mjs` (zero dipendenze, `node:zlib` come `generate-icons.mjs`) somma i gzip di `dist/assets` e fallisce oltre 60 KB per il JS e 10 KB per il CSS. Alla posa: 53,1 KB e 3,8 KB — i tre livelli nuovi e le card si erano già mangiati 8 KB dal 45,3 misurato una settimana prima, il momento giusto per mettere il cricchetto. Alzare il tetto è lecito, sapendolo e dicendolo nella PR.
...
## ADR-053 — Le impostazioni dichiarate diventano vere, e il salvataggio impara a perdonare

- Stato: **Accettata**
- Data: 10 agosto 2026
- Contesto: `textScale`, `highContrast` e `dialectEnabled` erano dichiarate in `AccessibilitySettings`, salvate e validate dal primo giorno — **senza interfaccia né effetto**. QUALITY.md chiedeva da sempre il testo ridimensionabile al 200%. E il validatore del salvataggio era una mina: pretendeva tutti i campi delle impostazioni, quindi aggiungerne o toglierne uno invalidava ogni salvataggio esistente — scarto totale, silenzioso, chiave mai rimossa.
- Decisione, in tre parti:
  1. **La mina si disinnesca prima di camminarci sopra**: le impostazioni decodificano campo per campo, con il default per ciò che manca o non valida (`tolerantSettings`). Il **run resta severo**: un run corrotto è il caso in cui scartare è giusto. Con questo, ogni evoluzione futura delle preferenze è retrocompatibile per costruzione.
  2. **`textScale` e `highContrast` diventano veri.** La scala vive sul root (`data-text-scale`: 87,5% / 100% / 118%) — tutta l'interfaccia è in rem, quindi scala tutto tranne il canvas pixel-art, che è dimensionato dal layout; il contrasto è un set alternativo di token (`data-contrast="high"`: inchiostro pieno su fondo più scuro, bordi visibili). Controlli veri nel menù («Dimensione del testo», «Contrasto elevato»), e **il livello sopravvive**: entrambe entrano nel percorso senza-render di ADR-050, perché sono presentazione pura sul root del documento.
  3. **`dialectEnabled` esce dal modello.** Implicava un catalogo di battute in dialetto con fallback per ogni capitolo: contenuto, non codice — materiale da sequel semmai, non una promessa da tenere nei tipi. La decodifica tollerante rende la rimozione indolore per i salvataggi che portano ancora la chiave.
- Verifica: salvataggio senza un campo delle impostazioni che riprende coi default e il run intatto; campo invalido che torna al default senza veto; scala e contrasto applicati sul root **senza rimontare il livello** (stesso nodo host, posizione intatta); end-to-end che imposta Grande + contrasto a livello vivo, verifica gli attributi sul root e **scandisce con axe il tema a contrasto elevato**; il run corrotto resta scartato.
- Conseguenza: il menù impostazioni passa da due sezioni a quattro (personaggio, testo, visualizzazione, audio); la voce n. 8 delle verifiche manuali di QUALITY.md si aggiorna; l'eccezione di ADR-050 si allarga a ogni impostazione di pura presentazione, e la sua lista è il posto dove le prossime dovranno giustificarsi.

## ADR-054 — Il service worker mantiene le promesse: offline atomico, aggiornamenti annunciati, versione visibile

- Stato: **Accettata**
- Data: 10 agosto 2026
- Contesto: il worker scritto a mano prometteva l'offline e non lo garantiva — la lista di precache conteneva le pagine legali ma **non i bundle con hash, cioè il gioco**: il primo caricamento offline dopo un deploy dipendeva da cosa la cache di runtime aveva visto per caso, e lo scenario reale era lo schermo bianco. `skipWaiting()` sostituiva il worker in silenzio a metà partita; la cache di runtime cresceva per sempre; Netlify serviva `sw.js` e `index.html` senza regole di cache. Il proprietario ha chiesto inoltre una versione visibile e un modo per forzare l'aggiornamento.
- Decisione, in cinque parti:
  1. **Il worker si genera al build.** Il sorgente vive come template (`src/platform/pwa/sw-template.ts`, funzione pura e testata); il plugin Vite riempie la lista di precache con i bundle reali e deriva `CACHE_NAME` dall'hash del bundle: **ogni deploy ha la sua cache completa e atomica**, e l'attivazione elimina tutte le altre — il che chiude anche la crescita illimitata. Il vecchio `public/sw.js` non esiste più.
  2. **L'aggiornamento aspetta il giocatore.** Niente `skipWaiting` all'installazione: il worker nuovo resta in attesa, la pagina mostra il banner «Nuova versione disponibile — tocca per aggiornare» (fuori dal mount, così i re-render non lo toccano), e al tocco il worker prende il controllo e la pagina si ricarica **una volta sola**. La primissima installazione non annuncia niente: non è un aggiornamento.
  3. **La versione è visibile** (richiesta del proprietario): il build id finisce in un meta della pagina al `writeBundle` — il momento deterministico, dopo che Vite ha emesso l'html — e il menù la mostra («Versione: CDnwzyRN»). In sviluppo dice «dev», che è esattamente ciò che è.
  4. **«Controlla aggiornamenti»** nel menù: chiede al browser di riscaricare il worker adesso; se c'è una versione in attesa la applica e ricarica, altrimenti «Sei già all'ultima versione». È la valvola di sfogo per i casi alla ADR-046: chi sospetta di essere indietro si sblocca da solo, senza cancellare i dati.
  5. **Netlify smette di incollare la shell**: `no-cache` su `sw.js`, `/` e `index.html`; gli asset con hash restano immutabili un anno.
- Verifica: template riempito e senza `skipWaiting` nell'install (test sul sorgente generato); annuncio del worker in attesa, mai alla prima installazione; ricarica esattamente una; controllo manuale che applica il pending; banner nascosto finché non serve e accettazione a un tocco; end-to-end nel preview costruito con versione visibile e «Sei già all'ultima versione» dal worker vivo; prova offline manuale sulla deploy preview prima del merge, come da checklist.
- Conseguenza: è l'ultima voce della fase 3. Un worker rotto si porta in tasca, quindi questa PR viaggia da sola; se un deploy futuro dovesse fallire l'attivazione, il nome-cache per build fa sì che il deploy successivo riparta pulito.

## ADR-055 — Ingaggio: il primo indizio insegna, lo scatto si vede, la curva si raddrizza

- Stato: **Accettata**
- Data: 11 agosto 2026
- Contesto: chiusa la fase dei bug, il proprietario vuole che i dieci livelli si sentano meglio senza aggiungerne altri. Tre difetti misurati, non impressioni:
  1. **Il livello 1 non insegnava niente.** Il primo indizio stava a 668px, oltre il primo fossato: un giocatore poteva attraversare mezzo livello senza capire che gli indizi si raccolgono e che la riga di stato li conta.
  2. **La carica dello scatto era invisibile.** `sprintCharge` è calcolata a ogni frame dal ADR-029 e **non l'ha mai disegnata nessuno**: il livello 2, l'unico i cui fossati _richiedono_ la corsa, era un indovinello — non sapevi di essere a mezzo secondo dall'averla.
  3. **La curva si sfaldava proprio sul finale.** I tre livelli con superpotere non dichiaravano `sprint`: tre ruoli su quattro perdevano la corsa imparata al livello 2 esattamente nei tre livelli più lunghi. E i checkpoint si diradavano mentre la letalità saliva — fino a 1690px senza bandierina nel castello, il livello con il robot in pattuglia.
- Decisione, tre interventi minimi:
  1. **Il primo indizio nel corridoio d'apertura**: `photo` si sposta sulla mensola già presente a 300px. Un salto solo, nei primi secondi, e la riga di stato passa da «0 di 3» a «1 di 3» — l'obiettivo spiegato giocandolo, senza una riga di tutorial. Una riga di configurazione.
  2. **La barra della carica**: la stessa barra da 2px del carburante del drone, sotto il personaggio, che si riempie mentre tieni la direzione e lampeggia quando la corsa è pronta. Solo presentazione: legge stato già puro, nessuna modifica al modello.
  3. **La curva raddrizzata**: `sprint` restituito a superstar, parco e castello (stessa taratura di ADR-029 — il Varano mantiene la differenza col suo potere, che carica in 0,4s invece di 0,9 e sopravvive al cambio di direzione); e tre bandierine nuove dove il tratto scoperto era più lungo — prima della tribuna stampa, prima della ronda del furgoncino e dopo il corridoio del robot.
- **Le invarianti sono la parte che dura.** Due test nuovi in `arcade-variety`: gli ultimi tre livelli devono dichiarare lo scatto, e **nessun tratto senza bandierina supera i 1500px** su tutti e dieci. Il secondo ha trovato subito il 1690 del castello — la prova che l'invariante serviva. E la planata in scatto è promossa da `zona-model` a tutti i livelli, con un'esclusione **di principio**: «Le chat di paese» ha per progetto fossati impossibili senza la corsa (ADR-029, inchiodato da `sprint-model.test.ts`), quindi lì il pozzo _è_ il punto e una camminata distratta deve costare una vita.
- Verifica: le due invarianti sopra su tutti i livelli registrati; test dell'adapter che la barra viene disegnata mentre carica; simulazioni senza poteri ancora a zero cadute su ogni livello; browser reale sul livello 1 (indizio nei primi secondi) e sul livello 2 (barra che si riempie).

## ADR-056 — La card di fine livello: il momento del livello, non solo della campagna

- Stato: **Accettata**
- Data: 11 agosto 2026
- Contesto: finire un livello con 3 indizi su 3, la stella presa col potere acceso, il cameo avvistato e zero vite perse valeva **un ciuffo di particelle, una riga e 700 ms di pausa**. Il pannello del punteggio compariva una volta sola, alla fine dell'intera campagna; `LevelOutcome` non trasportava né la stella né il cameo, quindi quei risultati **non esistevano da nessuna parte** dopo l'istante in cui accadevano. Per un gioco che vuole essere rigiocato e condiviso, era il buco più grande rimasto.
- Decisione:
  1. **Il contratto ricorda**: `LevelOutcome` guadagna `bonusCollected` e `cameoSeen`. Il totale di campagna li somma in OR — la cartolina finale dice «una stella è stata presa da qualche parte», la card del livello parla del livello.
  2. **La card di fine livello**, gemella di quella di KO: stesso `role="dialog"`, stessa cornice, chiave di celebrazione. Mostra la posizione in campagna, punteggio, indizi, vite rimaste, e fino a quattro riconoscimenti guadagnati — _Tutti gli indizi_, _Nessuna vita persa_, _Stella della Leggenda_ (solo dove il livello ne ha una) e _Cameo avvistato_.
  3. **Aspetta il giocatore.** Il piano prevedeva un avanzamento automatico per non introdurre clic obbligatori nei test; alla prova è una premessa sbagliata — una schermata di premi che si auto-chiude in due secondi nega sé stessa, e la card di KO aspetta già da ADR-041. Nessun timer decide: la celebrazione di 700 ms resta, poi la card prende il posto e sta lì. Non è un timer nel senso WCAG perché non c'è nessun timer.
...
## ADR-058 — La documentazione dice quello che il gioco fa

- Stato: **Accettata**
- Data: 13 agosto 2026
- Contesto: la specifica pubblica è stata scritta prima del gioco, e per undici mesi il gioco l'ha superata senza che nessuno tornasse indietro a correggerla. Il divario misurato, a campagna chiusa: il README dichiarava «milestone corrente M1P — Livello 1» con dieci livelli pubblicati; `ARCHITECTURE.md` descriveva un layer `features/`, `campaign.ts`, `compose-packs.ts` e `it-bs.ts` che non sono mai esistiti, sette fasi di `AppPhase` invece di tre, e le impostazioni con due campi rimossi da ADR-053; `QUALITY.md` e `CONTENT_MODEL.md` chiedevano di validare «48 combinazioni `role × approach × sensitivity × storyScope`» quando ADR-048 ha lasciato in piedi solo il ruolo; `EXPANSIONS.md` progettava in dettaglio un Dossier Origini mai costruito; `SOURCES.md` elencava nove schede di Dossier che il pack non contiene; `PRIVACY.md` prometteva al giocatore un interruttore «Condividi statistiche anonime» che nell'interfaccia non c'è.
- Decisione: riallineare tutta la documentazione pubblica allo stato reale, con tre regole.
  1. **Ciò che non è stato fatto esce.** Presets del giocatore, hook di estensione elencati come pubblicati, `composeStoryPacks()`, `OutcomeRule` con priorità, capitoli del pack `origins`, schede di Dossier previste, catalogo dialettale: cancellati, non rimandati. Un piano non realizzato scritto al presente è una bugia con la data sbagliata.
  2. **Ciò che esiste ma non è usato viene detto tale.** `storyScope`, `ChapterInsertion`, `CoreChapterTransition`, i contratti `DossierCard`, i nodi `scene`/`surprise`/`dossier-card` e i quattro SVG del prototipo restano nel codice come cuciture, ma ogni documento ora dichiara che nessuna schermata li raggiunge. Una cucitura dichiarata è un'opzione; una cucitura taciuta è codice morto.
  3. **La fonte di verità è il codice.** Gli elenchi che tendevano a divergere — regole del validatore, flussi end-to-end, soglie di coverage, azioni ed effetti del dominio — ora puntano al file che li contiene invece di duplicarne il contenuto in prosa.
